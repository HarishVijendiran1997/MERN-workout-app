...
const WorkoutDetails = ({ workout }) => {
    ...
    return (
        <div className="flex items-start bg-white dark:bg-darkSecondary max-w-screen rounded-lg font-Poppins ml-4  pb-5  pl-5 pt-5 flex-col shadow-md mb-5">
            {isEditing ? (
                <div>
                    ...
                    <div className="flex flex-wrap gap-2.5 justify-between">
                        ...
                        <input className="border p-1 rounded-lg border-gray-300 dark:border-darkBorder mr-2 dark:bg-darkInputBackground dark:text-darkInputText" placeholder="Workout Title" value={title} onChange={handleTitleOnChange} />
                        <input className="border p-1 rounded-lg border-gray-300 mr-2 dark:border-darkBorder 
                        dark:bg-darkInputBackground dark:text-darkInputText" type="number" min='1' placeholder="Load" value={load} onChange={handleLoadOnChange} />
                        <input className="border p-1 rounded-lg border-gray-300 mr-2 dark:border-darkBorder 
                        dark:bg-darkInputBackground dark:text-darkInputText" type="number" min='1' placeholder="Reps" value={reps} onChange={handleRepsOnChange} />
                        <p className="dark:text-darkTextResult">Last updated: {formatDistanceToNow(new Date(workout.updatedAt), { addSuffix: true })} ago</p>
                ...
                <h4 className="cursor-pointer text-2xl font-extrabold text-blue-600 hover:text-darkTextPrimaryHover dark:text-darkTextPrimary" onClick={handleTitleClick}>{workout.title}</h4>
                <div className="flex justify-between w-full pr-5 items-center dark:text-darkTextResult">
                    <p><strong className="dark:text-darkTextSecondary">Load (kg) : </strong>{workout.load}</p>
                    <p><strong className="dark:text-darkTextSecondary">Reps : </strong>{workout.reps}</p>
                    <p><strong className="dark:text-darkTextSecondary">Created : </strong>{formatDistanceToNow(new Date(workout.createdAt), { addSuffix: true })}</p>


                    <div className="flex gap-2 mt-3">
                        <svg onClick={handleEnableEdit} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className=" text-white rounded-lg p-2 cursor-pointer bg-blue-600 hover:bg-blue-800 dark:bg-darkEditButton dark:hover:bg-darkEditButtonHover dark:text-darkButtonText transition duration-200 size-10">
                            <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                        </svg>

                        <svg onClick={confirmDelete} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="text-white rounded-full p-2 cursor-pointer bg-red-500   hover:bg-red-600 dark:text-darkButtonText dark:bg-darkDeleteButton dark:hover:bg-darkDeleteButtonHover dark:active:bg-darkDeleteButton transition duration-200 size-10">
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 flex items-center justify-center backdrop-blur-lg bg-black/80">
                    <div className="bg-white dark:bg-darkSecondary p-6 rounded-lg shadow-lg">
                        <p className="text-lg font-semibold dark:text-darkTextResult">
                            Are you sure you want to delete this workout?
                        </p>
                        <div className="flex justify-end space-x-4 mt-4">
                            <button
                                onClick={confirmDeleteCancel}
                                className="px-4 py-2 bg-gray-400 hover:bg-gray-500 active:bg-gray-400 dark:text-darkCancelButtonText dark:bg-darkCancelButton dark:hover:bg-darkCancelButtonHover dark:active:bg-darkCancelButton rounded-lg cursor-pointer"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={handleDelete}
                                className="cursor-pointer px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 dark:text-darkButtonText dark:bg-darkDeleteButton dark:hover:bg-darkDeleteButtonHover dark:active:bg-darkDeleteButton"
                            >
                                Confirm
                            </button>
                        </div>
